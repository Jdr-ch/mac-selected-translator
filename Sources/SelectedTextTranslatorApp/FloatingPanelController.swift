...
final class FloatingPanelController {
    ...
    func showLoading(_ message: String) {
        show(title: "划词翻译", body: message, autoHideAfter: nil)
    }

    /// Shows the translated text and keeps it selectable for copy/paste.
    func showResult(_ translation: String) {
        show(title: "翻译结果", body: translation, autoHideAfter: 14)
    }

    /// Shows an actionable error from either macOS permission checks or backend calls.
    func showError(_ message: String) {
        show(title: "翻译失败", body: message, autoHideAfter: 8)
    }

    private func show(title: String, body: String, autoHideAfter seconds: TimeInterval?) {
