...
菜单中的“工作场景…”打开采用 B 方案的卡片面板：左侧是桌面和保存状态，中部是项目／群组摘要，右侧是状态和独立操作。点击整行文字或空白只查看明细，复选框单独控制勾选，行内采集／保存／恢复按钮只执行自身动作。默认勾选全部已保存桌面，新增桌面可通过“刷新桌面”加入清单。下方窗口详情以 tabs 切换该桌面的窗口，查看完整网址、群组、WebStorm 项目路径及布局，继续支持“选择项目…”、“重新绑定桌面…”和移除窗口。每个窗口标题及 tab 按恢复成功／失败显示绿色／红色背景，采集缺项显示黄色并列出窗口名称、缺少信息和处理方式。采集和恢复期间，卡片背景显示循环光带，仅表示运行状态，不表示完成百分比；隐藏面板或启用减少动态效果时停止动画。
...
恢复前集中准备目标桌面，然后最多并发处理 4 个窗口；WebStorm 项目按模板顺序串行恢复，前一项目打开和定位完成或失败后再执行下一项，窗口等待上限为 120 秒。Chrome 群组修改与同应用启动采用独占入口，其他应用仍可并行。恢复复用现有项目和群组、补齐缺少标签，保留用户额外标签。某个窗口失败后其他项继续，明细展示失败阶段和原因，可仅重试所选桌面的失败项；不自动运行开发服务，也不新增登录后自动恢复。

每个桌面的场景窗口全部恢复成功后，自动对这些窗口执行一次“对齐”，复用菜单中的应用缩放限制和排列规则，最终布局以对齐结果为准，不改写保存的模板。自动对齐明确绑定目标桌面，不依赖鼠标或当前焦点，不调整无关窗口。有窗口恢复失败时先跳过，失败项重试全部成功后再对齐；仅对齐失败时，重试不会重新打开已成功的窗口。
