...
菜单中的“工作场景…”打开采用 B 方案的双行卡片面板：左侧是桌面和保存状态，中部是项目／群组摘要，右侧是状态和独立操作。点击整行文字或空白可切换勾选，行内采集／保存／恢复按钮只执行自身动作。默认勾选全部已保存桌面，新增桌面可通过“刷新桌面”加入清单。下方窗口详情可切换该桌面的窗口，查看完整网址、群组、WebStorm 项目路径及布局，继续支持“选择项目…”、“重新绑定桌面…”和移除窗口。采集和恢复期间，卡片背景显示循环光带，仅表示运行状态，不表示完成百分比；隐藏面板或启用减少动态效果时停止动画。

Chrome 使用独立目录 `/Users/jiangdengrui/Documents/AI/chrome-extension` 中的 Manifest V3 扩展，安装与单独维护步骤见该目录的 README。构建脚本将 `WorkspaceChromeHost` 打包进 App，`scripts/install_workspace_extension_host.py` 注册 Native Messaging 宿主。

模板只保存在 `~/Library/Application Support/SelectedTextTranslator/WorkspaceScene/scene.json`；每次保存只更新相应桌面，保留其他桌面与未保存草稿，也支持保存空桌面。恢复只读取勾选桌面的已保存版本，其他桌面有草稿时仍可恢复。桌面通过“桌面 N”名称和原显示器匹配，删除重建后的同名桌面可直接复用；缺失时通过调度中心按需追加创建，每次创建后回读确认，最多 16 个普通桌面／显示器。原显示器断开或跨显示器编号无法通过追加获得时，使用“重新绑定桌面…”将整桌面模板绑定并保存到新目标。

恢复前集中准备目标桌面，然后最多并发处理 4 个窗口；Chrome 群组修改与同项目启动采用独占入口，应用等待和就绪窗口定位可以并行。恢复复用现有项目和群组、补齐缺少标签，保留用户额外标签。某个窗口失败后其他项继续，可仅重试所选桌面的失败项；不自动运行开发服务，也不新增登录后自动恢复。

跨桌面分配使用 macOS 私有 Spaces 接口：Tahoe 优先提交异步窗口移动，旧系统使用兼容工作区映射。每次移动后回读实际桌面及窗口几何；系统升级后接口失效会显示失败。第一版采集标准窗口，隐藏、最小化、全屏空间和无法唯一识别的窗口需要先恢复为普通窗口；WebStorm 只按项目目录启动，内部编辑状态由 IDE 自身恢复。

WebStorm 项目通过 LaunchServices 的系统应用入口打开，不直接派生 IDE 可执行文件，避免 WebStorm／Qoder 等长期子进程继承本 App 的文稿访问身份。旧版本启动且仍在运行的 IDE／插件进程需要一次正常退出并重新打开，已运行进程的权限归属不会因替换 App 自动改变。窗口识别只读取 IDE 暴露的项目路径，不在每次等待轮询中重复访问项目目录。

每个 Chrome 场景条目独占一个浏览器窗口；群组已合并在同一窗口时，恢复会重新分到各自窗口，再放回目标桌面。扩展需更新并重新加载至 1.0.1 或更新版本，旧扩展会在恢复前提示升级。

扩展重新安装或本地存储重建后，旧场景仍可恢复：App 优先使用原连接，失效时核对唯一在线连接与原资料安装目录再重新连接，不必重新采集。资料不同或存在多个无法区分的连接时会明确提示；连接、打开群组、匹配系统窗口和验证布局的超时会分别说明失败阶段。

采集以 WindowServer 的全部桌面清单为准，通过后台读取补齐 `AXWindows` 未暴露的其他桌面窗口，无需逐个切换桌面。暂未响应的窗口会在预览中标为待处理并阻止所属桌面保存，其他完整桌面仍可保存；再次采集继续读取缺失窗口。桌面行分别显示窗口数量和运行结果。
