...
菜单中的“工作场景…”统一打开场景面板，在窗口内选择“采集桌面 3、4、5”或“恢复场景”。选择条目可查看 WebStorm 实际项目目录、Chrome 全部网址及群组、目标桌面和窗口布局；待补充的项目可用“选择项目…”修正。采集检查后点击“保存场景”，以后在同一窗口点击“恢复场景”即可恢复，失败条目可以单独重试。
...
模板只保存在 `~/Library/Application Support/SelectedTextTranslator/WorkspaceScene/scene.json`；重新保存会更新这个场景。桌面或显示器身份变化时，在面板重新绑定并保存。恢复会复用现有项目和群组、补齐缺少标签，保留用户额外标签；不自动运行开发服务，也不新增登录后自动恢复。
...
采集以 WindowServer 的全部桌面清单为准，通过后台读取补齐 `AXWindows` 未暴露的其他桌面窗口，无需逐个切换桌面。暂未响应的窗口会在预览中标为待处理并阻止保存；再次采集继续读取缺失窗口。状态栏分别列出桌面 3、4、5 的窗口数量。
